...
import kotlinx.coroutines.rethrow
...
abstract class BaseJellyfinMediaSource(
    ...
    private suspend fun getCachedResponse(cacheKey: String): SearchResponse? {
        return cacheMutex.withLock {
            ...
            return@withLock if (System.currentTimeMillis() - timestamp < CACHE_EXPIRY.inWholeMilliseconds) {
                response
            } else {
            ...
            query.subjectNames
                ...
                .flatMapConcat { subjectName ->
                    try {
                        ...
                        logger.error(e) { "搜索主题 '$subjectName' 时发生错误" }
                        emptyFlow()
                    }
                }
                ...
                .mapNotNull { item ->
                    try {
                        createMediaMatch(item, query)
                    } catch (e: Exception) {
                        logger.error(e) { "创建MediaMatch对象失败: item.Id=${item.Id}, item.Type=${item.Type}" }
                        null
                    }
                }
    ...
    private suspend fun fetchAllSeriesContent(seriesList: List<Item>) = seriesList.asFlow()
        .flatMapMerge { series ->
            try {
                ...
                // 如果没有季，直接返回空流
                if (seasons.isEmpty()) {
                    logger.warn { "系列 ${series.Name} (ID: ${series.Id}) 没有任何季" }
                ...
                seasons.asFlow()
                    .flatMapMerge { season ->
                        try {
                            ...
                            // 如果没有集，记录警告但继续处理其他季
                            if (episodes.isEmpty()) {
                                logger.warn { "季 ${season.Name} (ID: ${season.Id}) 没有任何集" }
                            }
                            
                            episodes.asFlow()
                        } catch (e: Exception) {
                            logger.error(e) { "获取季 ${season.Id} 的集失败" }
                            emptyFlow()
                        }
                    }
            } catch (e: Exception) {
                logger.error(e) { "获取系列 ${series.Id} 的季失败" }
                emptyFlow()
            }
        }
    ...
    private suspend fun createMediaMatch(item: Item, query: MediaFetchRequest): MediaMatch? {
        try {
            val (originalTitle, episodeRange) = when (item.Type) {
                "Episode" -> {
                    val indexNumber = item.IndexNumber ?: run {
                        logger.warn { "Episode缺少IndexNumber: ${item.Name} (ID: ${item.Id})" }
                        return null
                    }
                    Pair(
                ...
                "Movie" -> Pair(
                    ...
                    logger.warn { "不支持的Item类型: ${item.Type} (ID: ${item.Id})" }
                    return null
                }
            }
                ...
                media = DefaultMedia(
                    ...
                    extraFiles = MediaExtraFiles(
                        subtitles = try {
                            ...
                            logger.error(e) { "获取字幕失败: item.Id=${item.Id}" }
                            emptyList()
                        }
                    ),
            ...
            logger.error(e) { "创建MediaMatch对象时发生未预期的错误: item.Id=${item.Id}, item.Type=${item.Type}" }
            return null
        }
    }
    ...
    private suspend fun doSearch(
        ...
        // 缓存未命中，执行实际搜索
        val response = retry(MAX_RETRIES) {
            try {
                client.use {
                    get("$baseUrl/Items") {
                ...
                logger.error(e) { "搜索失败: subjectName=$subjectName, parentId=$parentId, includeItemTypes=$includeItemTypes" }
                if (it < MAX_RETRIES - 1) {
                    logger.info(e) { "搜索重试: ${e.message}" }
                    delay(1.seconds)
                }
                throw e
            }
